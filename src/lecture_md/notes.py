"""Lecture-note generation: turn per-slide transcripts into handout pages."""
...
from lecture_md.slides_md import (
    TRANSCRIPT_RE,
    extract_json_object,
    load_json_records,
    parse_sections,
...
def asr_text(record: dict[str, Any]) -> str:
    for key in ("asr_transcript", "api_asr_transcript", "local_asr_transcript", "mimo_asr_transcript"):
...
def call_note(
    *,
    ...
    system = (
        "你是课程讲义整理助手。你的任务是把逐页课堂转写整理成适合复习的讲义稿。"
        "去掉口头禅、重复、自我修正和课堂杂音，但保留老师讲解中的推理顺序和关键限定。"
        "不要编造 PPT 和转写之外的信息；如果信息不足，要明确保持简略。"
        "数学符号、公式、集合、表项和算法符号必须用 Markdown/LaTeX 格式表达。"
        "只输出严格 JSON。"
    )
    terms_text = terms.strip()
    terms_section = f"课程术语：\n{terms_text}\n\n" if terms_text else ""
    user = f"""请把这一页整理成精辟讲义稿。

要求：
- 输出的是“讲义稿”，不要写成逐字稿，也不要写“老师说”。
- 删除口语化重复，例如“好”“然后呢”“大家看”“这个这个”等。
- 公式、符号和表项用 Markdown/LaTeX，例如 `$f(x) = x^2$`、`$O(n \\log n)$`、`A[i, j]`。
- 字母、编号、专有名词尽量按 PPT OCR 和上下文校正。
...
"""
    ...
    if not content:
        raise ValueError(f"Empty model content for {slide_id}")
    try:
